<%@page import="com.hybrid.decision_tree"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
   <%@ page import="java.util.*,java.io.*,java.util.TreeMap,com.hybrid.*" %>  
<!doctype html>
<html lang="en">
   ...
   // decision_tree.map.clear();
		try {
			...
			while (line != null) {
				...
				try{
					if(decision_tree.map.containsKey(line.split(",")[8])){
					 spam=bayesNetwork.bayesNetwork.get(line.split(",")[8]);
					
					out.println("<td>"+spam+"</td>");
					}else{
						spam="unknown";
						out.println("<td>unknown</td>");
					}
					if(spam.equals("ham")){
					if(spam.equals(line.split(",")[9])){
						tp++;
					}else{
						tn++;
					}
	    ...
	    float accuracy =(tp+fn)/(tp+tn+fp+fn);
	    float FPR =fp/(tn+fp);
	    ...
	    session.setAttribute("name", "bayes Network");
